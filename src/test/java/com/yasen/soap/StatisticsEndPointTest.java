...
public class StatisticsEndPointTest {
	...
	void getRankingGameDataTest() throws IOException {
		...
		StringSource expectedResponse = new StringSource("<ns2:RankingDataSoapList xmlns:ns2=\""+NAME_SPACE
			    + "\"><ns2:RankingDataSoap>"
        ...
        client.sendRequest(withPayload(request)).andExpect(noFault())
				.andExpect(validPayload(new ClassPathResource("hangman.xsd"))).andExpect(payload(expectedResponse))
				.andExpect(xpath("/ns2:RankingDataSoapList/ns2:RankingDataSoap[1]/ns2:rankType", NAMESPACE_MAPPING)
			            .evaluatesTo("monthly"));
	}
	...
	void getRankingUserDataTest() throws IOException {
		...
		StringSource request = new StringSource(
				"<tns:UserRankingRequest xmlns:tns='"+NAME_SPACE+"'>"
					+"<tns:username>test123</tns:username>"
					+"<tns:pageNum>0</tns:pageNum>"
					+ "</tns:UserRankingRequest>");
		
		StringSource expectedResponse = new StringSource("<ns2:UserProfileSoap xmlns:ns2=\""+NAME_SPACE +"\">"
        ...
        client.sendRequest(withPayload(request)).andExpect(noFault())
				...
				.andExpect(xpath("/ns2:UserProfileSoap/ns2:statusValues", NAMESPACE_MAPPING)
			            .evaluatesTo("1"));
	}
